...
//		int[] arr = {20,35,-15,7,-22,1,55};
		...
		for (int i = 1; i < arr.length; i++) {
			int key = arr[i]; // take first element
			int j = i -1; // the initial index
			while(j>=0 && arr[j]>key) {				
				arr[j+1] = arr[j];
				j = j-1;
			}
...
//			// tale element at place 1
//			int newEle = a[i];  //first element
//			int j2; // stores least int in index
//			for (j2 = i; j2>0 && a[j2-1] > newEle; j2--) {
//				a[j2] = a[j2-1]; // shift forward/. At first shift nothing happens, next -15 will be moved all the way down copy 35 to -15, 20 35 
//				count++;
//			}
//			// in place of 20 i.e first element place -15
//			a[j2] = newEle;
//		}
//		for (int i = 0; i < a.length; i++) {
